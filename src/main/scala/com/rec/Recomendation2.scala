...
case class StrIDRating(uID: String, mID: String, value: Double)


class Recomendation2 {
...
//    )
  ...
  def main(args: Array[String]) {
    ...
    model.recommendProductsForUsers(num).flatMap(x=>{
      val array: Array[org.apache.spark.mllib.recommendation.Rating] = x._2
      val list = new Array[String](array.length);
      var i = 0;
      array.foreach(x => {
        val uid = x.user
        val product = x.product
        val rating = x.rating
        val s = uidTable.value.get(uid)+"\t"+midTable.value.get(product)+"\t"+rating;
        list(i) = s;
        i=i+1
      })
      list
